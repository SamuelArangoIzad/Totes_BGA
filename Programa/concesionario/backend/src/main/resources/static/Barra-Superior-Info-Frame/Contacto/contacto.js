...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const horaInput = document.querySelector("#hora-cita");

    scheduleButton.addEventListener("click", function () {
        const title = "Cita de Asesoría - Totes Concesionario";
        const description = "Te esperamos para asesorarte en la compra de tu auto ideal.";
        const location = "Concesionario Totes, Bucaramanga";
        const gmail = "ctapruebathunder@gmail.com"; // Correo del calendario

        // Obtener la fecha y hora seleccionada
        const fecha = fechaInput.value; // yyyy-mm-dd
        const hora = horaInput.value;   // hh:mm

        if (fecha && hora) {
            // Convertimos la fecha y hora al formato YYYYMMDDTHHMMSS
            ...
            const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${startDate}/${endDate}&details=${encodeURIComponent(description)}&location=${encodeURIComponent(location)}&add=${gmail}`;

            // Redireccionamos al usuario
            window.open(url, "_blank");
        } else {
            alert("Por favor selecciona la fecha y hora para agendar tu cita.");
        }
    });
});